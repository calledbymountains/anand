...
class SAM3(BaseNet):
    def __init__(self, pretrained_model=None):
        super(SAM3, self).__init__('SAM3', num_classes=4, input_shape=26, pretrained_model=pretrained_model)

    def logits(self, input_batch):
        layer1 = tf.layers.dense(inputs=input_batch, units=16, kernel_initializer=tf.initializers.glorot_normal, activation=tf.nn.sigmoid,
                                 name='h1')


        layer2 = tf.layers.dense(inputs=layer1, units=32,
